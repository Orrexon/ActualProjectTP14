...
struct App
{
	void create()
	{
		...
		music.play();
		music.setLoop(true);
		windowManager = new WindowManager("Create object - Lingonstyle", 800, 600);
		open = true;
		draw_lines = true;
		drags = false;
		startedDraw = false;
	...
	void saveObject()
	{
		...
		if (!savepath.empty())
		{
			...
			imageNode["path"] = FileSystem::getRelativePath(root.c_str(), texture_path.c_str());

			std::cout << imageNode["path"].asString() << std::endl;

			imageNode["path"] = String::replace(imageNode["path"].asString(), "\\\\", "/");
			std::cout << imageNode["path"].asString() << std::endl;
			...
			if (ofs.is_open())
			{
				...
				MessageBoxA(windowManager->getWindow()->getSystemHandle(), "Success", message.c_str(), 0);
			}
		}
		else
		{
			MessageBoxA(windowManager->getWindow()->getSystemHandle(), "The savepath couldn't be located. Try another path or contact Lingonskogen's Lead programmer.", "Fail", 0);
		}
	}
